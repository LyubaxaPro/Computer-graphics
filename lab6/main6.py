...
import time
from funcs6 import create_line, fill_figure, put_borders, create_circle
...
def fill(self):
    ...
    if self.checkBox.checkState() == 0:
        fill_figure(self, False)
    else:
        fill_figure(self, True)

    put_borders(self, self.bg_color)
    add_pixmap(self)
    stop_time = time.clock()
    self.time_line.setText(str(round((stop_time - start_time), 6)) + " сек")
